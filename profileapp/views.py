...
def facebook(request):
    ...
    if request.method == "POST":
        if request.POST.get('pageid'):
            ...
            model_path = "D:/Done/profileapp/sccnmodelfiles/cnnenglishmodel.tflite"
...
def bert_analysis_fb(tweets):
    ...
    vocab = np.load(
        'D:/Done/profileapp/sccnmodelfiles/bertEnglishVocab.npy', allow_pickle=True).item()
    custom_objects = {'TFBertModel': TFBertModel}
    bert_model_path = 'D:/Done/profileapp/sccnmodelfiles/bertEnglish.h5'
...
def classifyTweet(request):
    ...
    if request.method == "POST":
        ...
        if language == "English":
            ...
            model_path = "D:/Done/profileapp/sccnmodelfiles/converted_model_urdu3.tflite"
            # tweet_text = tweet
...
def prepare_input(text):
    ...
    vocabulary = np.load(
        r'D:/Done/profileapp/sccnmodelfiles/EnglishCNNVocab.npy', allow_pickle=True).item()
    cleaned = []
...
def bert_analysis(tweets):
    # Load the BERT model and tokenizer from the transformers library
    vocab = np.load(
        'D:/Done/profileapp/sccnmodelfiles/EnglishCNNVocab.npy', allow_pickle=True).item()
    custom_objects = {'TFBertModel': TFBertModel}
    bert_model_path = 'D:/Done/profileapp/sccnmodelfiles/bertEnglish.h5'
    model = load_model(bert_model_path, custom_objects=custom_objects)
...
def prepare_input_urdu(text):
    ...
    vocabulary = np.load(
        r'D:/Done/profileapp/sccnmodelfiles/UrduCNNVocb.npy', allow_pickle=True).item()
    cleaned = []
    for word in sentences_padded:
        for word2 in word:
            if word2 in vocabulary:
